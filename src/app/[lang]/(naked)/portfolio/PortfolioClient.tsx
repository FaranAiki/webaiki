...
export default function PortfolioClient({ 
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 print:bg-white dark:print:bg-slate-950 print:text-slate-900 dark:print:text-slate-100">
      ...
      <nav className="sticky top-0 z-50 bg-slate-950/80 backdrop-blur-md border-b border-slate-800 p-4 print:hidden">
        <div className="container mx-auto flex justify-between items-center">
          ...
          <button 
            ...
            className="flex items-center gap-2 bg-cyan-600 hover:bg-cyan-500 text-white px-4 py-2 rounded-lg transition-colors font-medium relative z-[100]"
          >
            <Download size={18} />
      ...
      <main className="container mx-auto max-w-4xl p-8 md:p-12 space-y-12 print:p-0 print:max-w-none">
        {/* Header Section */}
        <header className="flex flex-col md:flex-row gap-8 items-center md:items-start border-b border-slate-800 pb-12 print:border-slate-200 dark:print:border-slate-800">
          <div 
          ...
          <div className="flex-1 text-center md:text-left space-y-4">
            <div>
              <h1 className="text-4xl md:text-5xl font-bold text-white print:text-slate-900 dark:print:text-white">Muhammad Faran Aiki</h1>
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-y-2 gap-x-6 text-slate-400 text-sm print:text-slate-600 dark:print:text-slate-400">
        ...
        <section className="space-y-4">
          <h2 className="text-2xl font-bold flex items-center gap-3 text-white print:text-slate-900 dark:print:text-white">
            <BookOpen className="text-cyan-500" />
            {dict.About_Me}
          </h2>
          <div 
            className="text-slate-300 leading-relaxed print:text-slate-700 dark:print:text-slate-300"
            ...
            <section className="space-y-6">
              ...
              <div className="space-y-8">
                {workHighlights.map((work, idx) => (
                  <div key={idx} className="relative pl-6 border-l-2 border-slate-800 print:border-slate-200 dark:print:border-slate-800">
                    <div className="absolute -left-[9px] top-1 w-4 h-4 bg-cyan-500 rounded-full border-4 border-slate-950 print:border-white dark:print:border-slate-950"></div>
                    <div className="space-y-1">
                      <h3 className="font-bold text-lg text-slate-100 print:text-slate-900 dark:print:text-100">{work.title}</h3>
                      <div className="flex justify-between text-sm">
                        <span className="text-cyan-400 font-medium">{work.company}</span>
                        <span className="text-slate-500">{work.period}</span>
                      </div>
                      <p className="text-slate-400 text-sm mt-2 leading-relaxed print:text-slate-600 dark:print:text-slate-400">{work.description}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-6">
                {education.map((edu, idx) => (
                  <div key={idx} className="space-y-1">
                    <h3 className="font-bold text-lg text-slate-100 print:text-slate-900 dark:print:text-100">{edu.degree}</h3>
                    <div className="flex justify-between text-sm">
                      <span className="text-cyan-400 font-medium">{edu.school}</span>
                      <span className="text-slate-500">{edu.period}</span>
                    </div>
                    <p className="text-slate-400 text-sm print:text-slate-600 dark:print:text-slate-400">{edu.description}</p>
                  </div>
                ))}
              </div>
            </section>
                ...
                {projectHighlights.map((project, idx) => (
                  <div key={idx} className="space-y-2">
                    <div className="flex justify-between items-start">
                      <h3 className="font-bold text-lg text-slate-100 print:text-slate-900 dark:print:text-100">{project.title}</h3>
                      {project.link && (
                    ...
                    <p className="text-slate-400 text-sm leading-relaxed print:text-slate-600 dark:print:text-slate-400">{project.description}</p>
                    {project.image && (
                      <div 
                  ...
                  <div key={idx} className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {skillGroup.items.map((skill, sIdx) => (
                        <span 
                          key={sIdx}
                          className="px-3 py-1 bg-slate-900 border border-slate-800 rounded-full text-xs text-slate-300 print:bg-slate-100 dark:print:bg-slate-900 print:border-slate-200 dark:print:border-slate-800 print:text-slate-700 dark:print:text-slate-300"
                        >
                          {skill}
                        </span>
                      ))}
        ...
        {/* Footer for PDF */}
        <footer className="hidden print:block border-t border-slate-200 dark:border-slate-800 pt-8 mt-12 text-center text-slate-500 text-xs">
