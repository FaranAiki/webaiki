                                ...
                                <div className="relative w-full h-full max-w-[400px] max-h-[400px] aspect-square flex justify-center items-center overflow-hidden transform-gpu rounded-3xl">
                                    {job.image && job.image.length > 0 ? (
                                        job.image[0].toLowerCase().endsWith('.pdf') ? (
                                            ...
                                            <Image src={job.image[0]} alt={`${job.title} at ${job.company}`} fill className="object-contain transition-transform duration-700 hover:scale-[1.03] scale-[1.01]" sizes="(max-width: 768px) 50vw, 400px" quality={75} priority={true} />
                                        )
                                    ) : (
                            ...
                            {job.image && job.image.length > 0 ? (
                                <div className="absolute inset-0 z-0">
                                    {job.image[0].toLowerCase().endsWith('.pdf') ? (
                                        ...
                                        <Image src={job.image[0]} alt={`${job.title} at ${job.company}`} fill sizes="(max-width: 768px) 50vw, 100vw" quality={75} className={`object-cover transition-all duration-1000 dark:brightness-[0.35] brightness-[1.1] grayscale-[0.2] opacity-20`} priority={true} />
                                    )}
                                    ...
                                    <div className="aspect-[4/5] relative rounded-2xl overflow-hidden shadow-theme-shadow transition-all duration-700 group-hover:rotate-0 rotate-3 group-hover:scale-105">
                                        {job.image && job.image.length > 0 ? (
                                            job.image[0].toLowerCase().endsWith('.pdf') ? (
                                                ...
                                                <Image src={job.image[0]} alt={`${job.title} at ${job.company}`} fill sizes="(max-width: 768px) 50vw, 400px" quality={75} className="object-cover" priority={true} />
                                            )
                                        ) : (
                                            <PlaceholderIcon company={job.company} />
                                        )}
