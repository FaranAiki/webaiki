                    ...
                    <div className="relative aspect-video mb-6 rounded-xl overflow-hidden bg-theme-surface-strong">
                        {job.image && job.image.length > 0 ? (
                            job.image[0].toLowerCase().endsWith('.pdf') ? (
                                ...
                                <ClientOnlyImage src={job.image[0]} alt={`${job.title} at ${job.company}`} fill sizes="(max-width: 768px) 50vw, 330px" quality={60} className="object-cover transition-transform group-hover:scale-105" priority={priorityImages ? idx < 4 : false} placeholder="blur" blurDataURL={shimmer600x400} />
                            )
                        ) : (
